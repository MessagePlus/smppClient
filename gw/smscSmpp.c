...
static SMPP *smpp_create(Octstr *smppbox_id, Octstr *our_host, Octstr *log_file, long log_level, long reconnect_delay,
		Octstr *host, int transmit_port, int receive_port, Octstr *system_type, Octstr *username, Octstr *password,
		Octstr *address_range, int source_addr_ton, int source_addr_npi, int dest_addr_ton, int dest_addr_npi,
		int enquire_link_interval, int max_pending_submits, int version, int priority, int validity, Octstr *my_number,
		int smpp_msg_id_type, int autodetect_addr, Octstr *alt_charset, Octstr *alt_addr_charset, Octstr *service_type,
		long connection_timeout, long wait_ack, int wait_ack_action, int esm_class) {
	SMPP *smpp;
...
static long convert_addr_from_pdu(Octstr *id, Octstr *addr, long ton, long npi, Octstr *alt_addr_charset) {
	...
	case GSM_ADDR_TON_INTERNATIONAL:
		...
		if (octstr_len(addr) < 7) {
			...
			warning(0, "SMPP[%s]: Mallformed addr `%s', generally expected at least 7 digits. ", octstr_get_cstr(id),
...
static Msg *pdu_to_msg(SMPP *smpp, SMPP_PDU *pdu, long *reason) {
	...
	*reason = SMPP_ESME_ROK;
	...
	/* check source addr */
	if ((*reason = convert_addr_from_pdu(smpp->smppbox_id, pdu->u.deliver_sm.source_addr, ton, npi,
	...
	if (pdu->u.deliver_sm.destination_addr == NULL) {
		error(0, "SMPP[%s]: Mallformed destination_addr `%s', may not be empty. "
				"Discarding MO message.", octstr_get_cstr(smpp->smppbox_id),
	...
	if ((*reason = convert_addr_from_pdu(smpp->smppbox_id, pdu->u.deliver_sm.destination_addr, ton, npi,
			smpp->alt_addr_charset)) != SMPP_ESME_ROK)
		goto error;
	...
	if (pdu->u.deliver_sm.esm_class & ESM_CLASS_SUBMIT_UDH_INDICATOR) {
		...
		if (udhl > octstr_len(msg->sms.msgdata)) {
			error(0, "SMPP[%s]: Mallformed UDH length indicator 0x%03x while message length "
					"0x%03lx. Discarding MO message.", octstr_get_cstr(smpp->smppbox_id), udhl,
...
static Msg *data_sm_to_msg(SMPP *smpp, SMPP_PDU *pdu, long *reason) {
	...
	*reason = SMPP_ESME_ROK;
	...
	/* check source addr */
	if ((*reason = convert_addr_from_pdu(smpp->smppbox_id, pdu->u.data_sm.source_addr, ton, npi, smpp->alt_addr_charset))
			!= SMPP_ESME_ROK)
		goto error;
	...
	/* check destination addr */
	if ((*reason = convert_addr_from_pdu(smpp->smppbox_id, pdu->u.data_sm.destination_addr, ton, npi,
			smpp->alt_addr_charset)) != SMPP_ESME_ROK)
		goto error;
...
static SMPP_PDU *msg_to_pdu(SMPP *smpp, Msg *msg) {
	...
	pdu->u.submit_sm.service_type =
			octstr_len(msg->sms.binfo) ? octstr_duplicate(msg->sms.binfo) : octstr_duplicate(smpp->service_type);
	...
	if (smpp->source_addr_ton > -1 && smpp->source_addr_npi > -1) {
		...
		debug("bb.sms.smpp", 0, "SMPP[%s]: Manually forced source addr ton = %d, source add npi = %d",
				octstr_get_cstr(smpp->smppbox_id), smpp->source_addr_ton, smpp->source_addr_npi);
	} else {
		...
		if (octstr_get_char(pdu->u.submit_sm.source_addr, 0) == '+') {
			if (!octstr_check_range(pdu->u.submit_sm.source_addr, 1, 256, gw_isdigit)) {
				...
				if (smpp->alt_addr_charset) {
					...
					} else if (charset_convert(pdu->u.submit_sm.source_addr, SMPP_DEFAULT_CHARSET,
	...
	if (smpp->dest_addr_ton > -1 && smpp->dest_addr_npi > -1) {
		...
		debug("bb.sms.smpp", 0, "SMPP[%s]: Manually forced dest addr ton = %d, dest add npi = %d",
				octstr_get_cstr(smpp->smppbox_id), smpp->dest_addr_ton, smpp->dest_addr_npi);
	} else {
	...
	if (validity != SMS_PARAM_UNDEFINED) {
		...
		pdu->u.submit_sm.validity_period = octstr_format("%02d%02d%02d%02d%02d%02d000+", tm.tm_year % 100,
				tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	}

	if (msg->sms.deferred != SMS_PARAM_UNDEFINED && msg->sms.deferred > 0) {
		struct tm tm = gw_gmtime(time(NULL) + msg->sms.deferred * 60);
		pdu->u.submit_sm.schedule_delivery_time = octstr_format("%02d%02d%02d%02d%02d%02d000+", tm.tm_year % 100,
				tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	}
...
//		{
//			debug("bb.sms.smpp", 0, "SMPP[%s]: throughput limit exceeded (%.02f,%.02f)", octstr_get_cstr(smpp->smppbox_id), load_get(smpp->load, 0), smpp->conn->throughput);
//			break;
//		}
		debug("bb.sms.smpp", 0, "SMPP[%s]: throughput (%.02f)", octstr_get_cstr(smpp->smppbox_id),
				load_get(smpp->load, 0));
...
#ifdef HAVE_LIBSSL
	if (smpp->use_ssl)
	conn = conn_open_ssl(smpp->host, smpp->receive_port, smpp->ssl_client_certkey_file, smpp->our_host);
	else
#endif
...
static Msg *handle_dlr(SMPP *smpp, Octstr *destination_addr, Octstr *short_message, Octstr *message_payload,
		Octstr *receipted_message_id, long message_state, Octstr *network_error_code) {
	Msg *dlrmsg = NULL;
	...
	if (smpp->version > 0x33 && receipted_message_id) {
		...
		case -1: /* message state is not present, partial SMPP v3.4 */
			...
			break;
		default:
			warning(0, "SMPP[%s]: Got DLR with unknown 'message_state' (%ld).", octstr_get_cstr(smpp->smppbox_id),
					message_state);
			dlrstat = DLR_FAIL;
			break;
		}
	...
	if (msgid == NULL || network_err == NULL || dlrstat == -1) {
		...
		if (respstr) {
			...
			ret = sscanf(octstr_get_cstr(respstr), "id:%64[^s] sub:%d dlvrd:%d submit date:%14[0-9] done "
					"date:%14[0-9] stat:%15[^t^e] err:%3[^t]", id_cstr, &sub, &dlrvrd, sub_d_cstr, done_d_cstr,
					stat_cstr, err_cstr);
			if (ret == 7) {
	...
	if (dlrmsg != NULL) {
		...
		error(0, "SMPP[%s]: got DLR but could not find message or was not interested "
				"in it id<%s> dst<%s>, type<%d>", octstr_get_cstr(smpp->smppbox_id), octstr_get_cstr(tmp),
...
//		mutex_unlock(smpp->conn->flow_mutex);
		...
		if (pdu->u.data_sm.esm_class & (0x04 | 0x08 | 0x20)) {
			...
			dlrmsg = handle_dlr(smpp, pdu->u.data_sm.source_addr, NULL, pdu->u.data_sm.message_payload,
					...
					pdu->u.data_sm.network_error_code);
			if (dlrmsg != NULL) {
				if (dlrmsg->sms.meta_data == NULL)
					dlrmsg->sms.meta_data = octstr_create("");
...
//		mutex_unlock(smpp->conn->flow_mutex);
		...
		if (pdu->u.deliver_sm.esm_class & (0x04 | 0x08 | 0x20)) {
			...
			dlrmsg = handle_dlr(smpp, pdu->u.deliver_sm.source_addr, pdu->u.deliver_sm.short_message,
					pdu->u.deliver_sm.message_payload, pdu->u.deliver_sm.receipted_message_id,
	...
	case submit_sm_resp:
		...
		if (smpp_msg == NULL) {
			warning(0, "SMPP[%s]: SMSC sent submit_sm_resp "
					"with wrong sequence number 0x%08lx", octstr_get_cstr(smpp->smppbox_id),
		...
		if (pdu->u.submit_sm_resp.command_status != 0) {
			error(0, "SMPP[%s]: SMSC returned error code 0x%08lx (%s) "
					"in response to submit_sm.", octstr_get_cstr(smpp->smppbox_id),
					pdu->u.submit_sm_resp.command_status, smpp_error_to_string(pdu->u.submit_sm_resp.command_status));
...
//			bb_smscconn_send_failed(smpp->conn, msg, reason, octstr_format("0x%08lx/%s", pdu->u.submit_sm_resp.command_status, smpp_error_to_string(pdu->u.submit_sm_resp.command_status)));
			--(*pending_submits);
		} else {
			Octstr *tmp;
			...
			if (smpp->smpp_msg_id_type == -1) {
				...
				if ((smpp->smpp_msg_id_type & 0x01)
					...
					tmp = octstr_format("%llu",
							strtoll(/* hex */octstr_get_cstr(pdu->u.submit_sm_resp.message_id), NULL, 16));
				} else {
					tmp = octstr_format("%llu",
							strtoll(/* decimal */octstr_get_cstr(pdu->u.submit_sm_resp.message_id), NULL, 10));
				}
			}

			/* SMSC ACK.. now we have the message id. */
			if (DLR_IS_ENABLED_DEVICE(msg->sms.dlr_mask))
				dlr_add(smpp->smppbox_id, tmp, msg);
...
//		mutex_unlock(smpp->conn->flow_mutex);
	...
	case generic_nack:
		...
		if (smpp_msg == NULL) {
			error(0, "SMPP[%s]: SMSC rejected last command, code 0x%08lx (%s).", octstr_get_cstr(smpp->smppbox_id),
					cmd_stat, smpp_error_to_string(cmd_stat));
		} else {
			msg = smpp_msg->msg;
			smpp_msg_destroy(smpp_msg, 0);

			error(0, "SMPP[%s]: SMSC returned error code 0x%08lx (%s) in response to submit_sm.",
					octstr_get_cstr(smpp->smppbox_id), cmd_stat, smpp_error_to_string(cmd_stat));
...
//			bb_smscconn_send_failed(smpp->conn, msg, reason, octstr_format("0x%08lx/%s", cmd_stat, smpp_error_to_string(cmd_stat)));
			--(*pending_submits);
		}
		break;
	...
	while ((key = gwlist_extract_first(keys)) != NULL) {
		...
		if (smpp_msg != NULL && difftime(now, smpp_msg->sent_time) > smpp->wait_ack) {
			...
			case SMPP_WAITACK_RECONNECT: /* reconnect */
				...
				warning(0, "SMPP[%s]: Not ACKED message found, reconnecting.", octstr_get_cstr(smpp->smppbox_id));
				octstr_destroy(key);
			...
			case SMPP_WAITACK_REQUEUE: /* requeue */
				...
				if (smpp_msg != NULL) {
					warning(0, "SMPP[%s]: Not ACKED message found, will retransmit."
							" SENT<%ld>sec. ago, SEQ<%s>, DST<%s>", octstr_get_cstr(smpp->smppbox_id),
							(long) difftime(now, smpp_msg->sent_time), octstr_get_cstr(key),
...
//					bb_smscconn_send_failed(smpp->conn, smpp_msg->msg, SMSCCONN_FAILED_TEMPORARILY, NULL);
				...
				break;
			default:
				error(0, "SMPP[%s] Unknown clenup action defined 0x%02x.", octstr_get_cstr(smpp->smppbox_id),
						smpp->wait_ack_action);
				octstr_destroy(key);
				gwlist_destroy(keys, octstr_destroy_item);
				return 0;
			}
...
//#define IS_ACTIVE (smpp->conn->status == SMSCCONN_ACTIVE || smpp->conn->status == SMSCCONN_ACTIVE_RECV)
	conn = NULL;
	info(0, "Antes en el while");
	while (!smpp->quitting) {
		debug("smppclient", 0, "entra en el while para conectarse al smsc");
		if (transmitter == 1)
		...
		while (conn != NULL) {
			debug("smppclient", 0, "entra en el while cuando la coneccion es diferente de NULL");
...
//					timeout = smpp->enquire_link_interval;
				if (transmitter && gw_prioqueue_len(smpp->msgs_to_send) > 0 && smpp->throttling_err_time > 0
				...
				} else if (transmitter && gw_prioqueue_len(smpp->msgs_to_send) > 0
...
//				time(&last_response);
				...
				debug("bb.sms.smpp", 0, "SMPP[%s]: %s: break and shutting down", octstr_get_cstr(smpp->smppbox_id),
						__PRETTY_FUNCTION__);

				break;
			}
		}
		debug("smppclient",0,"despues del while cuando no existe coneccion");
		if (conn != NULL) {
			conn_destroy(conn);
			conn = NULL;
		}
		...
		if (!smpp->quitting) {
			error(0, "SMPP[%s]: Couldn't connect to SMS center (retrying in %ld seconds).",
					octstr_get_cstr(smpp->smppbox_id), smpp->reconnect_delay);
//			mutex_lock(smpp->conn->flow_mutex);
...
//#undef IS_ACTIVE
	...
	if (transmitter || smpp->transmitter == -1) {
		debug("bb.smpp", 0, "SMSCConn  shut down.");
...
//
//	smpp = conn->data;
//	conn->load = (smpp ? (conn->status != SMSCCONN_DEAD ? gw_prioqueue_len(smpp->msgs_to_send) : 0) : 0);
//	return conn->load;
//}
...
//		return -1;
//
//	debug("bb.smpp", 0, "Shutting down SMSCConn %s (%s)", octstr_get_cstr(conn->name), finish_sending ? "slow" : "instant");
//
...
//	}
//	else if (esm_class != ESM_CLASS_SUBMIT_DEFAULT_SMSC_MODE && esm_class != ESM_CLASS_SUBMIT_STORE_AND_FORWARD_MODE)
//	{
//		error(0, "SMPP: Invalid esm_class mode '%ld' in configuration. Switching to \"Store and Forward\".", esm_class);
//		esm_class = ESM_CLASS_SUBMIT_STORE_AND_FORWARD_MODE;
//	}
	smpp = smpp_create(smppbox_id, our_host, log_file, log_level, reconnect_delay, host, port, receive_port,
			system_type, username, password, address_range, source_addr_ton, source_addr_npi, dest_addr_ton,
			dest_addr_npi, enquire_link_interval, max_pending_submits, version, priority, validity, my_number,
			smpp_msg_id_type, autodetect_addr, alt_charset, alt_addr_charset, service_type, connection_timeout,
			wait_ack, wait_ack_action, esm_class);
...
//	conn->data = smpp;
//	conn->name = octstr_format("SMPP:%S:%d/%d:%S:%S", host, port, (!receive_port && transceiver_mode ? port : receive_port), username, system_type);
...
//	conn->status = SMSCCONN_CONNECTING;
	 ...
	 */
	if (port != 0) {
		info(0, "Init trasmitter port");
		smpp->transmitter = gwthread_create(io_thread, io_arg_create(smpp,
						(transceiver_mode ? 2 : 1)));
		if (smpp->transmitter == -1) {
			info(0, "FALLLO");
		}
		info(0, "Fin trasmitter port");
	}

	if (receive_port != 0) {
		info(0, "Init receiver port");
		smpp->receiver = gwthread_create(io_thread, io_arg_create(smpp, 0));
	}
	if ((port != 0 && smpp->transmitter == -1) || (receive_port != 0 && smpp->receiver == -1)) {
		error(0, "SMPP[%s]: Couldn't start I/O threads.", octstr_get_cstr(smpp->smppbox_id));
		smpp->quitting = 1;
		if (smpp->transmitter != -1) {
			gwthread_wakeup(smpp->transmitter);
